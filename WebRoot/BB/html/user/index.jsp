...
<body>
			...
			<ul class="layui-nav fly-nav-user">
				...
				<c:if test="${salary!=null}">
					<li class="layui-nav-item"><a class="fly-nav-avatar"
						href="home.bb?usersid=${myuser.usesid}"> <cite class="layui-hide-xs">${myuser.usersname}</cite>
						...
						<dl class="layui-nav-child">
							<dd>
								<a href="home.bb?usersid=${myuser.usesid}"><i class="layui-icon"
							...
							<dd>
								<a href="fanes.bb?usersid=${myuser.usesid}"><i class="layui-icon">&#xe770;</i>好友中心</a>
							</dd>
	...
	<div class="layui-container fly-marginTop fly-user-main">
		<ul class="layui-nav layui-nav-tree layui-inline" lay-filter="user">
			<li class="layui-nav-item"><a href="home.bb?usersid=${myuser.usesid}">
					<i class="layui-icon">&#xe68e;</i> 我的主页 </a>
			</li>
			...
			<li class="layui-nav-item"><a href="fanes.bb?usersid=${myuser.usesid}"> <i
				...
				<div class="layui-tab-content" style="padding: 20px 0;">
					<div class="layui-tab-item layui-show">
						...
						<c:forEach items="${userpost }" var="upost">
							<li><a class="jie-title" href="posts.bb?pid=${upost.pid }"
								target="_blank">${upost.ptitle }</a> 
								<i><fmt:formatDate value="${upost.pdate }" pattern="yyyy-MM-dd HH:mm" /></i><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p>
								<a href="updatepost.bb?pid=${upost.pid }">
									<span class="layui-badge layui-bg-green"> 编辑 </span>
								</a>
								<a href="udelecttopc.bb?pid=${upost.pid }&uid=${myuser.usesid}" onclick="return confirm('确认删除吗？')">
									<span class="layui-badge layui-bg-blue"> 删除 </span>
								</a>
								<em>${upost.preadings }阅/${upost.pcomments }答</em>
							</li>
						</c:forEach>
						</c:if>
						<c:if test="${empty userpost }">
							<li style="text-align: center;">
								<span> 您还没有发布博文哦，赶快去发布一篇吧~ </span>
							</li>
						</c:if>
						</ul>
						<div id="LAY_page"></div>
					...
					<div class="layui-tab-item">
						<ul class="mine-view jie-row">
							<c:if test="${not empty upclist }">
							<c:forEach items="${upclist }" var="upclist">
								<li><a class="jie-title" href="posts.bb?pid=${upclist.pid }"
									target="_blank">${upclist.ptitle }</a> <i>收藏于<fmt:formatDate value="${upclist.pdate }" pattern="yyyy-MM-dd HH:mm" /></i>
								</li>
							</c:forEach>
							</c:if>
							<c:if test="${empty upclist }">
								<li style="text-align: center;">
									<span> 您还没有收藏的博文哦~ </span>
								</li>
							</c:if>
						</ul>
	...
	<div class="fly-footer">
		<p>
			<a href="centreall.bb" target="_blank">知心博客</a><span style="color: #fff;">
